...
<div class="row">
	<div class="col-md-12">
		...
		<div class="alert alert-success" role="alert" id="title">
			<button class="btn btn-primary" onclick="javascript:location.href='score.php'">查看其他考卷</button>
			...
			<?php
				...
				/*標題那些*/
			 	...
			 	$curr_title = "SELECT * FROM score WHERE curr_id = '$curr'";
			 	$curr_sql = mysql_query($curr_title);
			 	$row = mysql_fetch_array($curr_sql);
			 	/*繳交人數*/
			 	$ap = "SELECT * FROM score_import WHERE import_curr = '$curr'";
		...
		<?php
		 	$curr = substr($_SERVER['QUERY_STRING'], 3);
		 	$sql = "SELECT * FROM score_import WHERE import_curr = '$curr'";
		 	...
		 	<table class="table table-striped">
				...
				<tr>
					...
					<td><?php echo $row['import_ver']; ?></td>
					<td><?php echo $row['import_book']; ?></td>
					<td><?php echo $row['import_less1']." ~ ".$row['import_less2'];; ?></td>
				</tr>
				<?php
		 	}
		 	?>
